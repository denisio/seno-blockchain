...
from seno.util.network import is_localhost, is_in_network
...
class SenoServer:
    def __init__(
        ...
        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSSenoConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.seno_ca_crt_path, self.seno_ca_key_path = seno_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.seno_ca_crt_path, self.seno_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
    ...
    def get_full_node_connections(self) -> List[WSSenoConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self) -> List[WSSenoConnection]:
        result = []
        for _, connection in self.all_connections.items():
            result.append(connection)
        return result
