...
from seno.ssl.create_ssl import generate_ca_signed_cert, get_seno_ca_crt_key, make_ca_cert
...
from seno.util.path import mkdir
from seno.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def create_all_ssl(root: Path):
    ...
    seno_ca_crt, seno_ca_key = get_seno_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, seno_ca_crt, seno_ca_key, False, overwrite=False)
...
def seno_minor_release_number():
    res = int(seno_version_number()[2])
...
def seno_init(root_path: Path):
    if os.environ.get("SENO_ROOT", None) is not None:
        print(
            f"warning, your SENO_ROOT is set to {os.environ['SENO_ROOT']}. "
            f"Please unset the environment variable and run seno init again\n"
