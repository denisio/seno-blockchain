...
from seno.plotting.create_plots import create_plots
from seno.consensus.block_creation import create_unfinished_block, unfinished_block_to_full_block
...
from seno.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from seno.util.config import load_config, save_config
...
from seno.util.wallet_tools import WalletTool
...
class BlockTools:
    ...
    def __init__(
        ...
        if len(self.pool_pubkeys) == 0 or len(farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `seno generate keys`")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("SENO_ROOT", "~/.seno2/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
