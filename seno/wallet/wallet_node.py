...
from seno.types.blockchain_format.coin import Coin, hash_coin_list
...
from seno.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from seno.util.path import mkdir, path_from_root
from seno.wallet.block_record import HeaderBlockRecord
...
from seno.wallet.util.backup_utils import open_backup_file
from seno.wallet.util.wallet_types import WalletType
from seno.wallet.wallet_action import WalletAction
from seno.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSSenoConnection):
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSSenoConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSSenoConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSSenoConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
