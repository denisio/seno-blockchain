...
import { AlertDialog, Flex, Card } from '@seno/core';
...
import { mojo_to_seno_string, seno_to_mojo } from '../../../util/seno';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_seno_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of seno in the blockchain at the current
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    const amount = seno_to_mojo(amount_input.value);
    const fee = seno_to_mojo(fee_input.value);
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send seno to coloured address. Please enter a seno
              address.
            </Trans>
          </AlertDialog>,
        ),
