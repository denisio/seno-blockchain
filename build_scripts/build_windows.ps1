...
Write-Output "   ---"
Write-Output "Get SENO_INSTALLER_VERSION"
# The environment variable SENO_INSTALLER_VERSION needs to be defined
$env:SENO_INSTALLER_VERSION = python .\build_scripts\installer-version.py -win
...
Write-Output "Chia Version is: $env:SENO_INSTALLER_VERSION"
Write-Output "   ---"

Write-Output "   ---"
Write-Output "Build seno-blockchain wheels"
Write-Output "   ---"
pip wheel --use-pep517 --extra-index-url https://pypi.chia.net/simple/ -f . --wheel-dir=.\build_scripts\win_build .

Write-Output "   ---"
Write-Output "Install seno-blockchain wheels into venv with pip"
Write-Output "   ---"
...
Write-Output "pip install seno-blockchain"
pip install --no-index --find-links=.\win_build\ seno-blockchain
...
Write-Output "   ---"
Copy-Item "dist\daemon" -Destination "..\seno-blockchain-gui\" -Recurse
...
electron-packager . Seno --asar.unpack="**\daemon\**" --overwrite --icon=.\src\assets\img\chia.ico --app-version=$packageVersion
Write-Output "   ---"
...
If ($env:HAS_SECRET) {
   ...
   Write-Output "   ---"
   signtool.exe timestamp /v /t http://timestamp.comodoca.com/ .\release-builds\windows-installer\SenoSetup-$packageVersion.exe
   signtool.exe verify /v /pa .\release-builds\windows-installer\SenoSetup-$packageVersion.exe
   }   Else    {
